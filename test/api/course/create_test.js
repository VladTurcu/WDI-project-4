...
describe('POST /api/courses', ()=> {
  ...
  it('should return the correct data', done => {
    api
      ...
      .send(courseData[0])
      .end((err, res) => {
        const courseItem = res.body;
        expect(courseItem.id).to.be.a('string');
        expect(courseItem.title).to.equal(courseData[0].title);
        expect(courseItem.image).to.equal(courseData[0].image);
        expect(courseItem.category).to.equal(courseData[0].category);
        expect(courseItem.description).to.equal(courseData[0].description);
        expect(courseItem.lessons[Array(7)]).to.equal(courseData[0].lessons[Array(7)]);
        done();
      });
  });
});
